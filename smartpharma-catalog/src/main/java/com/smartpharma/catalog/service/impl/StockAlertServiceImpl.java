...
import com.smartpharma.catalog.repository.StockAlertRepository;
...
public class StockAlertServiceImpl implements StockAlertService {
    ...
    public void generateExpiryAlerts(Long pharmacyId) {
        ...
        for (StockBatch batch : expiredBatches) {
            if (batch.getStatus() != com.smartpharma.catalog.entity.StockBatch.BatchStatus.DISCARDED) {
                try {
        ...
        for (StockBatch batch : expiringBatches) {
            ...
            if (batch.getStatus() == com.smartpharma.catalog.entity.StockBatch.BatchStatus.ACTIVE) {
                try {
                    Product product = batch.getProduct();
                    if (product == null) continue;
